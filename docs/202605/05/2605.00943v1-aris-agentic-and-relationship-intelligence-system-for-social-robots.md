...
title_zh: ARIS：用于社交机器人的代理式与关系智能系统
...
tldr: 针对社交机器人在多轮对话和社交关系推理方面的局限，本文提出ARIS框架。该系统集成了多模态推理、基于图谱的社交世界模型和检索增强生成（RAG）技术。通过在Pepper机器人上的实验验证，ARIS显著提升了机器人的感知智能、拟人化程度和用户喜爱度，为实现长期的、具有上下文感知能力的社交互动提供了有效的模块化架构。
selection_source: fresh_fetch
motivation: 现有的社交机器人系统在处理大规模多轮对话、社交关系推理及上下文关联方面仍面临挑战。
method: 提出ARIS框架，结合了用于维护用户关系的知识图谱社交世界模型、高效的RAG对话流水线以及协调视听动作的模块化代理架构。
result: 用户研究显示，相比大语言模型基准，ARIS在感知智能、活跃度、拟人化和好感度等多个维度均有显著提升。
conclusion: ARIS通过整合社交关系建模与高效检索机制，成功增强了社交机器人在复杂交互场景下的表现，并计划开源其实现。
---

## 摘要
基础模型推动了社交机器人技术的发展，实现了与用户更丰富的感知和交流互动。然而，目前的系统在大规模的多轮参与、社交关系推理以及基于上下文的对话方面仍面临挑战。我们提出了 ARIS（代理式与关系智能系统），这是一个为社交机器人设计的代理式 AI 框架，它在单一模块化架构中统一了多模态推理、基于图的社交世界模型（Social World Model）以及检索增强生成（RAG）。我们在机器人介导的双人对话场景中，使用 Pepper 机器人对 ARIS 进行了评估，并将其与大语言模型基准进行了对比。一项用户研究（N=23）表明，ARIS 在感知智能、生命感、拟人化和好感度方面显著更高。我们的贡献主要有三点：(1) 社交世界模型，通过知识图谱显式地映射和更新用户之间的社交关系，从而实现跨场景的社交推理和重新识别；(2) 一种高效的基于 RAG 的对话流水线，在对话历史增长到数千次交流时仍能保持有限的延迟，同时确保响应的相关性；(3) 系统集成以及在模块化代理架构中对这些组件进行的实证验证，该架构通过结构化 API 协调语音、视觉和物理动作。ARIS 的实现将在论文发表后开源。
