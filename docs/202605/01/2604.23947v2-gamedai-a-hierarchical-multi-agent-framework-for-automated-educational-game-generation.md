...
tldr: 本研究提出 GamED.AI，一个基于分层多智能体框架的自动化教育游戏生成系统。该系统利用 LangGraph 子图、质量门控和 Pydantic 模式，将教师提供的题目转化为符合教学目标的互动游戏。它支持 15 种交互机制，涵盖空间推理和布鲁姆分类法目标。实验表明，该系统在保持高验证通过率和模式合规性的同时，显著降低了 Token 消耗和成本，为快速生成高质量教育游戏提供了有效方案。
selection_source: fresh_fetch
motivation: 旨在解决将教学问题自动转化为具有教育意义且机械逻辑严密的互动游戏这一难题。
method: 采用基于 LangGraph 的分层多智能体框架，结合确定性质量门控和结构化 Pydantic 模式进行分阶段生成。
result: "系统实现了 90% 的验证通过率，且相比 ReAct 智能体减少了 73% 的 Token 消耗，单游戏成本仅为 0.46 美元。"
conclusion: 研究表明，分阶段的架构约束在确保生成质量和对齐性方面比单纯的提示词策略更为有效。
---

## 摘要
我们介绍了 GamED.AI，这是一个分层多智能体框架，可将教师提供的问题转化为完全可玩、具有教育学基础的教育游戏，并通过形式化机制合约进行验证。GamED.AI 基于基于阶段的 LangGraph 子图、确定性质量门（Quality Gates）和结构化 Pydantic 模式构建，支持两个模板系列，涵盖了涉及空间推理、程序执行和高阶布鲁姆分类法（Bloom's Taxonomy）目标的 15 种交互机制。在涵盖五个学科领域的 200 个问题上进行的评估显示，该系统实现了 90% 的验证通过率、98.3% 的模式合规性，且相比 ReAct 智能体减少了 73% 的 token 消耗（每款游戏从约 73,500 降至约 19,900 tokens），每款游戏的成本为 0.46 美元。在此模型配置下，这些结果表明，与单纯的提示策略相比，受阶段限制的架构结构与对齐质量的相关性更强。我们的演示允许与会者在 60 秒内从自然语言生成符合布鲁姆分类法的游戏，检查流水线每个阶段的质量门输出，并浏览涵盖所有 15 种机制类型的 50 款精选游戏库。
