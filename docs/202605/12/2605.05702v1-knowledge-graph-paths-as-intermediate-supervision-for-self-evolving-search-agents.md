...
## 摘要
自进化搜索智能体通过生成并解决自身的搜索任务，减少了对人工编写训练问题的依赖。我们基于搜索自博弈（Search Self-Play, SSP），这是一种典型的“提议者-求解者”（Proposer and Solver）框架，其中问题通过多步搜索和推理生成并解答。然而在实践中，SSP 面临两个瓶颈：提议者在缺乏关系上下文的情况下从孤立的答案实体构建问题，导致在早期自博弈训练中产生许多无效或无法验证的问题；而求解者仅接收二元结果奖励，丢弃了来自部分正确搜索轨迹的有用信号。我们通过重用知识图谱路径作为构建衍生的中间监督，同时用于问题构建和奖励塑造，从而解决了这两个瓶颈。首先，我们将问题构建建立在 LLM 引导的知识图谱子图之上，为提议者提供关系上下文。其次，我们观察到构建和解决多跳问题可能涉及重叠的中间实体：用于构思问题的“事实桥梁”可以为回答问题提供近似的航点（waypoints）。利用这种重叠，我们引入了航点覆盖奖励（Waypoint Coverage Reward, WCR），根据错误求解轨迹对构建路径上实体的覆盖程度给予分级的部分信用，同时保留对正确答案的全额奖励。在七个问答基准测试和九种模型配置中，我们的方法在所有配置下都比标准 SSP 提高了平均分数，其中在多跳问答任务上的提升尤为显著。这些结果表明，知识图谱路径可以作为轻量级的中间监督被重用，在无需额外特定任务的人工标注或手动标记过程步骤的情况下，提供关系引导和过程反馈。
