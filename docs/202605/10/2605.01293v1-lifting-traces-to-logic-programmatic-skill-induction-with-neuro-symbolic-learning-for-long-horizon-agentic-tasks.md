...
## 摘要
基于基础模型的智能体通常由于纯提示词推理的瞬时性，在长程规划方面面临挑战。虽然现有的技能归纳方法通过将经验蒸馏为与状态无关的参数化脚本来缓解这一问题，但它们无法捕捉在动态环境中稳健执行所需的条件逻辑。在本文中，我们提出了神经符号技能归纳（NSI），这是一个将交互轨迹提升为模块化、基于逻辑的程序的框架。通过合成显式控制流和动态变量绑定，NSI 使智能体能够发现“何时”以及“为何”采取行动。这种范式实现了高效的泛化，允许智能体从少样本示例中归纳技能，并灵活地适应未见过的目标。在一系列智能体任务上的实验表明，NSI 一致优于最先进的基准方法，使智能体能够自我进化为基于逻辑的技能架构师。
