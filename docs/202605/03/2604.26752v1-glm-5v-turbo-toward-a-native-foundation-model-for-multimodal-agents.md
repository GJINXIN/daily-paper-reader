...
tldr: GLM-5V-Turbo 是一款旨在提升多模态智能体能力的原生基础模型。它将多模态感知（如图像、视频、网页、GUI等）深度集成到推理、规划和工具调用中，而非仅作为辅助接口。通过优化模型设计、强化学习及工具链，该模型在多模态编程和视觉工具使用方面表现卓越，为构建高效多模态智能体提供了实践路径。
selection_source: fresh_fetch
motivation: 现有的智能体能力受限于语言推理，需要将多模态感知作为核心能力以应对复杂的真实环境交互。
method: 通过整合多模态感知、分层优化、强化学习及工具链扩展，将感知能力深度融入推理与执行全过程。
result: 模型在多模态编程、视觉工具调用及基于框架的智能体任务中表现强劲，同时保持了优秀的纯文本编程能力。
