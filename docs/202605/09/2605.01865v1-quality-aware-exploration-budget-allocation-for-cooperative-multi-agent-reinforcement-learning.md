...
title_zh: 协同多智能体强化学习中感知质量的探索预算分配
...
tldr: 针对协作多智能体强化学习中探索强度难以平衡的问题，本文提出了一种质量感知的探索预算分配框架。该框架通过回报调节的 Sigmoid 调度器动态调整全局探索强度，并利用奖励信号质量（RSQ）指标在智能体间分配预算，将探索重点放在信号更可靠的智能体上。结合后继距离（SD）作为内在奖励，该方法在 MPE、SMAX 等多个基准测试中显著提升了协调效率和任务性能。
selection_source: fresh_fetch
motivation: 解决多智能体强化学习中探索强度难以平衡以及不同智能体内在奖励信号可靠性差异导致的协调失效问题。
method: 结合回报调节的全局调度器（RCB）与基于信号质量（RSQ）的智能体预算分配机制，并采用后继距离作为内在奖励。
result: 在 MPE、SMAX 和 MABrax 等七个协作基准测试中，该方法均取得了顶尖的收益表现。
conclusion: 通过动态调整全局强度并根据信号可靠性优化个体探索预算，可以有效提升多智能体系统的协作探索效率。
---

## 摘要
协同多智能体强化学习 (MARL) 要求智能体在组合爆炸的状态-动作空间中发现联合策略，然而有效的协调配置极其罕见。内在动机通过新颖性奖励增强任务奖励，是驱动探索的一种流行方法，但其有效性取决于探索强度 $β$。过大的 $β$ 值会淹没任务信号并导致协调崩溃，而过小的值则会阻碍对稀有策略的发现。我们解决了两个互补的挑战：在训练过程中全局调整 $β$，以及在内在奖励信号可靠性不同的智能体之间分配探索预算。我们的框架结合了用于全局强度控制的收益条件 Sigmoid 调度 (RCB) 和针对每个智能体的奖励信号质量 (RSQ) 指标，后者将探索预算集中在具有可靠信号的智能体上。核心见解是，接收到噪声内在奖励的智能体应该减少探索强度，且这种分配可以根据信噪比统计数据自动确定。后继距离 (Successor Distance, SD) 是一种拟度量内在奖励，它自然地产生可区分的单智能体信号质量，从而完善了具有收敛性和序保持保证的框架。在七个协同基准测试（MPE、SMAX、MABrax）中，我们的方法在所有环境下都实现了顶级的收益。
