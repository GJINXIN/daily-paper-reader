...
score: 7.0
evidence: 大规模多智能体协作
tldr: 针对大规模多智能体协作中智能体发现效率低、精度差的问题，本文提出 GRAIL 框架。该框架通过微调的小语言模型（SLM）实现毫秒级意图解析，利用伪文档扩展增强语义密度，并采用 MaxSim 共振机制进行细粒度匹配。实验表明，GRAIL 在保证高精度的同时，将发现延迟缩短至 400ms 以内，比 LLM 基准快 79 倍以上，为构建实时“智能体互联网”提供了高效且可扩展的工业级解决方案。
selection_source: fresh_fetch
motivation: 现有的智能体发现方法在 LLM 解析带来的高延迟与传统向量检索导致的低精度之间难以取得平衡。
method: 提出 GRAIL 框架，结合了 SLM 标签预测、伪文档语义扩展以及基于 MaxSim 的细粒度示例匹配技术。
result: "在 AgentTaxo-9K 数据集上，GRAIL 将端到端延迟降低了 79 倍以上，且在 Recall@10 指标上显著优于传统向量搜索。"
conclusion: 该研究证明了通过轻量级模型增强索引和细粒度匹配，可以在大规模场景下实现实时且精准的智能体发现。
---

## 摘要
随着基于大语言模型（LLM）的智能体生态系统迅速扩张，高效且准确的智能体发现已成为大规模多智能体协作的关键瓶颈。现有方法通常面临两难境地：要么依赖重量级 LLM 进行意图解析，导致极高的延迟（通常超过 30 秒）；要么使用单一的向量检索，以牺牲语义精度为代价换取速度。为了弥补这一差距，我们提出了 GRAIL（基于粒度共振的智能体/AI 链接），这是一个在不牺牲准确性的情况下实现低于 400 毫秒发现延迟的新型框架。GRAIL 引入了三项关键创新：(1) SLM 增强预测，使用专门微调的小语言模型（SLM）取代通用 LLM 解析器，实现毫秒级的能力标签预测；(2) 伪文档扩展，通过合成查询增强智能体描述，提高语义密度以实现鲁棒的稠密检索；(3) MaxSim 共振，一种细粒度匹配机制，计算用户查询与离散智能体使用示例之间的最大相似度，有效缓解了语义稀释问题。在包含 9,240 个智能体的新型大规模数据集 AgentTaxo-9K 上进行的验证表明，与基于 LLM 解析的基准方法相比，GRAIL 将端到端发现延迟降低了 79 倍以上，同时在 Recall@10 指标上显著优于传统的向量搜索。该框架为实时的“智能体互联网”提供了一种可扩展的工业级解决方案。
