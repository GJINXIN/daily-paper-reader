...
## 摘要
授权大语言模型（LLM）驱动的智能体动态调用工具并访问受保护资源会引入显著的安全风险，且随着智能体参与多轮对话并向分布式协作扩展，这些风险会急剧增加。被攻破或恶意的智能体应用可能会篡改工具调用、伪造结果，或请求超出主体预期任务范围的权限；由于当前的委托授权流程缺乏对原始主体意图的可见性，这些行为在现有流程中可能无法被察觉。鉴于此，我们为持续智能体语义授权（CASA）做出了以下贡献。首先，我们提出了一种混合运行时强制执行模型，该模型结合了由零信任拦截层实现的确定性控制和语义控制。五种确定性控制对消息流实施结构和数据完整性保证，而语义检查层则评估工具调用选择是否与委托给智能体的预期任务一致。其次，不同于以往针对单轮交互的基于任务的访问控制（TBAC）技术，我们将语义层分解为两个阶段：i) 任务提取步骤，在拦截层从多轮对话中提取主体的目标；ii) 授权服务器上的任务-工具语义匹配步骤，评估所请求的工具是否适用于提取的任务。第三，我们扩展了在之前工作中引入的 ASTRA 数据集，通过生成包含多轮交互的新型对话-工具数据集，其中包含针对给定任务的相关和不相关工具调用。最后，我们提供了多轮对话下 TBAC 的首个实验结果。
