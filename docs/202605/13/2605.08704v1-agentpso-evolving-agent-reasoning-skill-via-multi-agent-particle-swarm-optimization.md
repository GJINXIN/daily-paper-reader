...
tldr: AgentPSO是一个受粒子群优化（PSO）启发的框架，旨在通过多智能体协作演化大语言模型的推理技能。该方法将每个智能体视为粒子，其状态为自然语言描述的推理技能，通过结合个人最佳、全局最佳及同伴反馈进行语义更新。实验证明，AgentPSO在数学和通用推理任务上显著优于静态及仅推理阶段的多智能体基准，且演化出的技能具有跨任务和跨模型的迁移能力。
selection_source: fresh_fetch
motivation: 现有多智能体推理方法多依赖推理时辩论，容易受错误同伴影响且智能体技能静态，无法在任务间持续进化。
method: 提出AgentPSO框架，将推理技能视为粒子状态，通过模拟PSO的更新机制在自然语言空间内迭代优化智能体的推理策略。
result: 在数学和通用推理基准测试中，AgentPSO显著提升了推理性能，且演化出的技能在不同基准和模型间表现出良好的迁移性。
conclusion: AgentPSO通过群体演化机制成功捕获了可重用的推理程序，为提升大语言模型的推理能力提供了一种无需参数微调的新路径。
---
