...
conclusion: Token级的动态协作能有效克服多智能体系统的结构性漏洞，使诚实模型在对抗环境下具备更强的纠偏与恢复能力。
---

## 摘要
多智能体大语言模型（LLM）架构日益依赖响应级聚合（如多数投票，MAJ）来提升推理上限。然而，在开放环境中，智能体极易受到隐蔽的上下文损坏（如针对性提示注入）的影响。我们揭示了当前多智能体系统中的一个关键结构性漏洞：当受损智能体形成局部多数时，响应级聚合会失效。由于投票聚合的是已形成的结论，它无法察觉中间逻辑的缺陷。为了克服这一系统性局限，我们提出了 Token 级轮询（RR）协作，即智能体在共享的自回归上下文中顺序交替生成。我们将这一过程形式化为离散时间动力系统，证明了 Token 级交替将聚合方式从脆弱的最终投票计数（线性求和）转变为动态交织的逻辑链（非线性算子乘积）。通过这一理论视角，我们证明了诚实模型的修复拉力可以压倒对抗性损坏，即使受损智能体占据多数。我们在多种推理基准上进行了详尽的实证评估，结果表明，虽然 MAJ 在受损智能体达到多数时会崩溃，但 RR 在远超这一临界阈值的情况下仍能保持稳健的准确性。
