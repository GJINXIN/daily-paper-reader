...
motivation: 现有的 RAG 系统受限于 LLM 的上下文长度，难以在海量外部知识中有效整合信息并处理超长上下文。
method: 提出一种模型无关的算法，通过过滤和组织 LLM 过去生成的中间思考构建长期记忆，并根据新查询检索相关思考。
result: "在 AcademicEval 等数据集上，该方法相比基准模型在 F1 分数上提升了至少 7.6%，胜率提升了 16%。"
conclusion: Thought-Retriever 赋予了 LLM 代理自进化的长期记忆能力，使其能够通过持续交互和深度思考处理更复杂的抽象查询。
---
