...
tldr: AI智能体开发依赖自然语言提示词，但提示词的模糊性常导致性能波动。本文提出Agent Mentor开源库，通过分析执行日志中的语义轨迹，识别导致不良行为的特征，并自动向系统提示词中注入修正指令。该方法在多个基准测试中显著提升了智能体的准确性，为自动化智能体治理提供了新思路。
selection_source: fresh_fetch
motivation: 解决AI智能体因提示词定义不精确或存在歧义而导致的性能不稳定和行为偏差问题。
method: 通过监控执行日志并识别与不良行为相关的语义特征，动态地向智能体系统提示词中注入修正性指令。
result: 在多种智能体配置和基准任务的实验中，该方法显著提升了任务执行的准确率，特别是在规范模糊的场景下效果显著。
conclusion: 证明了通过语义轨迹分析自动优化提示词是提升智能体性能的有效途径，为未来智能体治理框架提供了参考。
---

## 摘要
AI 智能体的开发在很大程度上依赖于自然语言提示词（prompting）来定义智能体的任务、知识和目标。这些提示词由大语言模型（LLM）解释，并由其主导智能体的行为。因此，智能体的性能容易受到因提示词表述不精确或模糊而产生的波动影响。识别并纠正此类问题不仅需要检查智能体的代码，还需要检查执行日志中反映的、在整个执行生命周期内生成的内部系统提示词。在本研究中，我们介绍了一种作为 Agent Mentor 开源库一部分实现的分析流水线，该流水线能够监控并增量式地调整定义另一个智能体行为的系统提示词。该流水线通过系统地向智能体知识中注入纠正性指令来提高性能。我们描述了其底层机制，特别强调了如何识别与非预期行为相关的语义特征，并利用这些特征推导出纠正性陈述。我们通过在三种示例智能体配置和基准任务上进行重复执行运行，评估了所提流水线的有效性。这些实验为在未来的智能体治理框架中实现此类指导流水线的自动化提供了初步探索。总体而言，该方法在不同配置下均表现出一致且可衡量的准确率提升，特别是在规范模糊性占主导地位的环境中。为了保证可复现性，我们已将代码作为 Agent Mentor 库的一部分开源。
