...
tldr: 针对自动化仓储中多智能体路径规划（MAPF）的闭环算法存在短视和缺乏全局保证的问题，本文提出了一种基于证书驱动的冲突搜索算法（CDCBS）。该方法引入“证书轨迹”作为冲突规避的后备方案，并利用“车队预算”机制过滤更新，确保了算法的完备性。同时，通过可继承的预算限制分解技术，降低了复杂场景下的计算规模。实验证明，CDCBS在稠密环境下比现有算法ACCBS具有更稳定的解质量和更高的计算效率。
selection_source: fresh_fetch
motivation: 现有的闭环MAPF算法在有限计算预算下容易因短视导致在稠密场景中性能下降，且难以维持全局一致性和利用组合结构。
method: 引入证书轨迹作为后备方案，利用车队预算过滤更新以保证完备性，并采用可跨时间步继承的预算限制分解技术。
result: 在基准测试中，CDCBS在稠密环境下比ACCBS表现出更稳定的解质量，且分解机制显著减小了有效智能体分组的规模。
conclusion: 通过证书驱动机制和可继承分解，该框架在保持闭环算法扩展性的同时，增强了全局性能保证和计算效率。
---
