...
tags: ["query:mas-ss"]
score: 7.0
evidence: 智能体维持连贯社会行为的长期记忆
tldr: GAAMA 是一种为 AI 智能体设计的图增强关联记忆系统，旨在解决多轮对话中长期记忆的结构化存储与检索难题。它通过三阶段流水线构建包含片段、事实、反思和概念四类节点的层次化知识图谱，并结合语义搜索与个性化 PageRank（PPR）算法进行检索。在 LoCoMo-10 基准测试中，GAAMA 表现优异，显著提升了智能体在复杂长程对话中的推理和个性化表现。
selection_source: fresh_fetch
motivation: 现有的 RAG 和向量检索方法难以捕捉多轮对话中的关联结构和层次化逻辑，且现有图方法在处理演化记忆时存在检索偏差。
method: 构建包含片段、事实、反思和概念节点的层次化图谱，并采用结合语义相似度与边缘感知个性化 PageRank 的混合检索机制。
result: "在 LoCoMo-10 基准测试中，GAAMA 取得了 78.9% 的平均奖励，显著优于 RAG (75.0%) 和 HippoRAG (69.9%) 等基准方法。"
conclusion: 将图遍历排名与语义搜索相结合，能有效增强智能体对长期记忆中结构化关联信息的提取能力。
---

## 摘要
在多轮会话中与用户交互的 AI 智能体需要持久的长期记忆，以维持连贯且个性化的行为。目前的方法要么依赖于扁平的检索增强生成（RAG），这会丢失记忆间的结构关系；要么使用记忆压缩和向量检索，无法捕捉多轮对话的关联结构。文献中虽有少量基于图的技术被提出，但它们仍面临枢纽节点主导检索以及对演化记忆的分层推理能力不足的问题。我们提出了 GAAMA，一种图增强关联记忆系统，通过三步流水线构建以概念为中介的分层知识图谱：(1) 从原始对话中逐字保留片段（episode）；(2) 基于大语言模型（LLM）提取原子事实和主题级概念节点；(3) 合成高阶反思（reflection）。生成的图包含四种节点类型（片段、事实、反思、概念），由五种结构化边类型连接，其中概念节点提供了与语义相似性互补的横向遍历路径。检索过程通过加法评分函数，将基于余弦相似度的 k-最近邻搜索与感知边类型的个性化 PageRank（PPR）相结合。在 LoCoMo-10 基准测试（涵盖 10 个多轮对话中的 1,540 个问题）中，GAAMA 实现了 78.9% 的平均奖励，优于经过调优的 RAG 基准（75.0%）、HippoRAG（69.9%）、A-Mem（47.2%）和 Nemori（52.1%）。消融实验表明，将基于图遍历的排序（个性化 PageRank）与语义搜索相结合，相比于纯图节点语义搜索，性能得到了持续提升（整体提升 1.0 个百分点）。
