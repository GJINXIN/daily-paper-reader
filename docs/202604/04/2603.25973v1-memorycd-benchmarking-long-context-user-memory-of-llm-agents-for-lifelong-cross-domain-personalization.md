...
tldr: MemoryCD是首个基于真实世界（Amazon Review）长周期、跨领域行为的大规模用户记忆基准测试。针对现有基准局限于合成对话和短会话的问题，该研究通过追踪用户多年的真实交互，构建了包含14种模型、6种记忆方法和4类个性化任务的评估体系。研究揭示了现有长上下文处理方法在模拟真实用户跨领域行为方面的不足，为终身个性化智能体提供了重要的测试平台。
selection_source: fresh_fetch
motivation: 现有的LLM记忆基准多依赖合成数据和短会话，缺乏对真实世界中长周期、跨领域用户行为的评估能力。
method: 利用Amazon Review数据集构建了包含12个领域、跨越多年的真实用户交互轨迹，并设计了4种个性化任务来评估14种LLM及6种记忆增强方法。
result: 实验分析表明，现有的长上下文处理和记忆方法在处理复杂的跨领域终身个性化任务时，仍远未达到用户满意的水平。
conclusion: MemoryCD填补了真实用户长记忆评估的空白，为开发具备跨领域终身个性化能力的LLM智能体提供了关键的基准和方向。
---
