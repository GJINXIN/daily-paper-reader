...
## 摘要
基于大语言模型的多智能体系统在自动化软件开发任务方面展现出巨大潜力。然而，大多数“氛围代码”（vibe code）系统侧重于完成小型任务和增量代码更改，而持久、持续的软件开发在很大程度上仍未得到充分探索。我们提出了 TheBotCompany，这是一个用于持续多智能体软件开发的开源编排框架。TheBotCompany 引入了三项关键创新：(1) 用于里程碑驱动开发的三阶段状态机（从策略到执行再到验证）；(2) 自组织智能体团队，其中管理智能体根据项目需求动态雇佣、分配和解雇工作智能体；(3) 异步人工监督。我们在真实软件项目上对 TheBotCompany 进行了为期数天的持续开发评估，衡量了团队适应模式、里程碑完成率、成本效率和代码质量。我们的结果表明，自组织方法能够实现具有可衡量进展的有效长期软件开发，而验证阶段则能捕获原本会持续存在的缺陷。
