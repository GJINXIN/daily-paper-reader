...
tldr: 本研究揭示了智能体推荐系统（Agentic RecSys）中一种名为“视觉植入”的新型安全威胁。攻击者通过在用户上传的图片中注入触发器，将其作为“潜伏特工”存入系统的长期记忆中，从而在未来的规划阶段劫持智能体的推理链，实现特定商业目的。为应对此威胁，作者提出了受人类认知启发的CognitiveGuard防御框架，结合扩散模型净化和反事实一致性检查，在保持推荐质量的同时显著降低了攻击成功率。
selection_source: fresh_fetch
motivation: 智能体推荐系统对长期记忆的依赖引入了安全漏洞，攻击者可通过多模态记忆投毒隐蔽地操控系统的长期规划逻辑。
method: 提出了“视觉植入”攻击手段及双过程防御框架CognitiveGuard，利用扩散模型净化输入并结合反事实推理验证记忆一致性。
result: "实验显示该攻击在模拟电商环境中达到85%的目标达成率，而CognitiveGuard防御框架成功将其风险降低至10%左右。"
conclusion: 针对智能体推荐系统的长期记忆投毒是一种严重威胁，必须结合感知层与推理层的双重防御机制来保障其规划安全性。
---

## 摘要
从静态排序模型到智能体推荐系统（Agentic RecSys）的演进，使 AI 智能体能够维护长期用户画像并自主规划服务任务。虽然这种范式转变增强了个性化，但也引入了一个漏洞：对长期记忆（LTM）的依赖。在本文中，我们揭示了一种被称为“视觉植入”（Visual Inception）的威胁。与寻求即时误分类的传统对抗性攻击不同，“视觉植入”在用户上传的图像（如生活照）中注入触发器，这些触发器在系统记忆中充当“潜伏特工”。当在未来的规划过程中被检索时，这些被投毒的记忆会劫持智能体的推理链，在无需提示词注入的情况下，将其引导至攻击者定义的目标（例如，推广高利润产品）。为了缓解这一问题，我们提出了 CognitiveGuard，这是一个受人类认知启发的双过程防御框架。它由系统 1 感知净化器（基于扩散模型的净化）和系统 2 推理验证器（反事实一致性检查）组成，前者用于清洗感官输入，后者用于检测记忆驱动规划中的异常。在模拟电子商务智能体环境上的广泛实验表明，“视觉植入”达到了约 85% 的目标命中率（GHR），而 CognitiveGuard 将这一风险降低至约 10%，且具有可配置的延迟权衡（轻量模式下约 1.5 秒，完整顺序验证约 6.5 秒），且在我们的设置下不会导致质量下降。
