...
## 摘要
大语言模型（LLM）智能体正越来越多地被部署为可扩展的用户模拟器，用于推荐系统评估。然而，现有的模拟器通常通过文本或结构化元数据感知推荐内容，而非真实用户浏览的视觉界面——这是一个关键差距，因为对推荐布局的注意力既是视觉驱动的，又是高度个性化的。我们研究了将视觉语言模型（VLM）的视觉注意力与特定用户的注视模式相对齐，是否能提高仿真的忠实度。对在轮播式推荐场景下收集的真实眼动追踪数据集的分析表明，用户表现出稳定的个体注视模式，且这些模式对点击行为具有很强的预测性。基于这一发现，我们提出了用于用户仿真的注视点对齐微调（FixATE）。我们的方法首先通过可解释性算子探测 VLM 的内部视觉注意力，以获得与人类注视点相当的槽位级相关性分布，然后学习个性化软提示（soft prompts），以引导模型的注意力转向每个用户的特征注视模式。在三种基于可解释性的探测算子和两种架构不同的 VLM 主干网络上的实验表明，注意力对齐和点击预测准确率均有一致的提升。这些结果表明，让模型“像用户一样看”是开发能更忠实地再现用户在推荐界面中感知和行动方式的模拟器的可行路径。
