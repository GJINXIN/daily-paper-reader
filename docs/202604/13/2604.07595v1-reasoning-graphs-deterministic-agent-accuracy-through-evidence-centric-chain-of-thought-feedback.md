...
motivation: 现有智能体在每次查询时都从零开始推理并丢弃思维链，导致处理相似问题时准确率低且输出波动大。
method: 引入推理图和检索图结构，通过证据中心化的反向遍历机制，将历史评价反馈给当前证据评估过程。
result: 在多跳问答基准测试中，该方法在不重训模型的情况下实现了准确率的稳步上升和方差的显著坍缩。
conclusion: 推理图通过结构化的上下文工程实现了智能体的自我进化和确定性增强，为构建高可靠推理系统提供了新方案。
---
