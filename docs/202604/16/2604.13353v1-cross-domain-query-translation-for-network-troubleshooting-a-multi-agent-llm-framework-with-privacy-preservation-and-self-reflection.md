...
tldr: 本研究针对私有网络故障排除中非技术用户与专家间的沟通障碍，提出了一种基于多智能体大语言模型（LLM）的跨域查询翻译框架。该框架采用分层架构，结合ReAct推理与自我反思机制，实现了用户查询的精准分类、隐私信息的语义保持匿名化，以及技术响应的通俗化翻译。通过在1万个跨行业场景中的验证，证明了该系统在保护隐私的同时，能有效提升网络故障诊断的效率与用户理解度。
selection_source: fresh_fetch
motivation: 旨在解决私有网络环境下，非技术用户与电信专家之间因专业知识差异导致的沟通鸿沟及隐私泄露风险。
method: 提出一种分层多智能体架构，利用ReAct推理、自我反思机制、语义保持匿名化技术及少样本学习实现查询翻译与隐私保护。
result: "在涵盖多个垂直行业的10,000个未知验证场景中，该框架成功实现了高精度的查询分类和有效的技术术语转换。"
conclusion: 该多智能体LLM框架为电信领域提供了一种兼顾诊断效用与隐私安全的跨域沟通解决方案。
---

## 摘要
本文提出了一种分层多智能体大语言模型（LLM）架构，旨在弥合私有网络环境中非技术终端用户与电信领域专家之间的沟通鸿沟。我们提出了一种跨域查询翻译框架，利用通过基于反思的多智能体推理进行协作的专用语言模型。该系统解决了三个关键挑战：（1）采用双阶段分层方法准确分类与电信网络问题相关的用户查询；（2）通过对语义相关的个人身份信息（PII）进行匿名化处理来保护用户隐私，同时保持诊断效用；（3）将技术专家的回复翻译成用户易于理解的语言。我们的方法采用了增强了自我反思机制的 ReAct 风格智能体以实现输出的迭代优化，采用了遵循 $k$-匿名性和差分隐私原则的语义保持匿名化技术，以及针对有限训练数据场景设计的少样本学习策略。该框架在涵盖多个垂直行业的 10,000 个此前未见的验证场景中进行了全面评估。
