...
tldr: 本研究针对现有视频生成系统语义不可靠的问题，提出了一种基于智能体的生成框架。该系统不直接生成像素，而是利用LLM构建结构化的时空事件图（GEST），并通过3D游戏引擎确定性地执行。通过将叙事规划与物理约束强制执行分离，并采用分层多智能体架构，该方法在物理真实性和语义一致性上显著优于传统的神经视频生成模型。
selection_source: fresh_fetch
motivation: 现有神经视频生成系统生成的视频虽然视觉效果好，但往往缺乏语义可靠性和物理真实性，且难以生成可验证的结构化描述。
method: 采用分层多智能体架构，通过受工具约束的LLM规划生成可执行的时空事件图（GEST），并利用3D引擎进行确定性渲染。
result: "该系统生成的视频在物理有效性（58%）和语义对齐度（3.75/5）上大幅领先于VEO 3.1和WAN 2.2等主流神经生成模型。"
conclusion: 将LLM的叙事推理能力与3D引擎的物理约束相结合，是实现高语义一致性和物理真实性视频生成的有效路径。
---
