...
tldr: 针对多智能体问答中路由机制固定且协作僵化的问题，本文提出 EvolveRouter 框架。该框架通过闭环协同进化过程，将基于图的查询路由与针对性的指令优化相结合，使路由诊断引导智能体改进，同时优化后的智能体为路由提供更清晰的监督。此外，引入自适应推理策略，根据路由加权的答案一致性动态确定协作规模。实验证明，该方法在五个基准测试中均优于现有 SOTA 路由基准，显著提升了推理性能与效率。
selection_source: fresh_fetch
motivation: 现有的多智能体路由方法通常在固定智能体池上优化且协作模式僵化，无法根据查询动态调整智能体数量或提升智能体自身能力。
method: 提出 EvolveRouter 框架，通过闭环协同进化机制同步优化路由与指令，并利用路由加权的答案一致性实现自适应协作推理。
result: 在五个问答基准测试中，EvolveRouter 在 F1 和精确匹配指标上均一致优于现有的 SOTA 路由基准。
conclusion: 路由与提示词的协同进化以及自适应协作策略能有效提升多智能体推理的性能与效率。
---
