...
title_zh: 单智能体与多智能体在屏幕协作学习行为自动化视频分析中的对比研究
...
## 摘要
屏幕学习行为为学生在学习过程中如何寻求、使用和创造信息提供了宝贵的见解。分析屏幕行为投入对于捕捉学生的认知和协作过程至关重要。视觉语言模型（VLMs）的最新发展为自动化多模态视频数据分析中通常需要的劳动密集型人工编码提供了新机遇。在本研究中，我们基于 ICAP 框架，对比了领先的闭源 VLM（Claude-3.7-Sonnet, GPT-4.1）和开源 VLM（Qwen2.5-VL-72B）在单智能体和多智能体设置下，对协作学习场景中屏幕录像进行自动编码的性能。具体而言，我们提出并比较了两种多智能体框架：1）一种三智能体工作流多智能体系统（MAS），该系统按场景分割屏幕视频，并利用光标感知的 VLM 提示及基于证据的验证来检测屏幕行为；2）一种受 ReAct 启发的自主决策 MAS，它通过交替迭代推理、类工具操作（分割/分类/验证）和观察驱动的自我修正，生成可解释的屏幕行为标签。实验结果表明，所提出的两种 MAS 框架均取得了可行的性能，在场景和动作检测任务中优于单一 VLM。值得注意的是，基于工作流的智能体在场景检测方面表现最佳，而自主决策 MAS 在动作检测方面表现最佳。本研究证明了基于 VLM 的多智能体系统在视频分析中的有效性，并为多模态数据分析提供了一个可扩展的框架。
