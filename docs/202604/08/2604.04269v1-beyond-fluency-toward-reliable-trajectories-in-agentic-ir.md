...
tldr: 随着信息检索从被动排序转向多步自主代理工作流，早期微小错误易引发连锁反应，导致推理与执行失调。本文分析了工业级代理系统的失效模式，涵盖规划、检索、推理和执行。为解决复合错误和虚假流畅性问题，作者提出了在交互单元设置验证门控和基于不确定性的主动弃权机制，强调过程正确性与可靠执行，旨在提升代理系统的轨迹完整性与因果归因能力。
selection_source: fresh_fetch
motivation: 代理式信息检索系统在长路径任务中容易出现早期错误累积，导致尽管语言流畅但实际执行与逻辑脱节。
method: 提出在每个交互单元设置验证门控，并倡导在校准不确定性的基础上实施系统性弃权机制。
result: 系统性地总结并分类了工业代理系统在规划、检索、推理和执行等环节的典型失效模式。
conclusion: 可靠的代理式信息检索系统必须优先考虑过程的正确性和有据可查的执行，而非仅仅追求表面合理的生成结果。
---
