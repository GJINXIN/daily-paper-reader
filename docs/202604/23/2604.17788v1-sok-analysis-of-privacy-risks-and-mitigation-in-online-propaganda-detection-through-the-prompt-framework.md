...
tags: ["query:mas-ss"]
score: 7.0
evidence: 在线宣传检测与社会话语分析
tldr: 本研究针对在线宣传检测中的隐私风险，分析了162项研究并提出了PROMPT框架。该框架通过量化隐私增益、性能损失和部署成本，将风险与缓解策略映射，并引入合规性评分以评估系统对GDPR等法规的遵循情况。实验揭示了隐私保护与检测精度之间的权衡关系，为构建合规且隐私感知的检测系统提供了技术基础。
selection_source: fresh_fetch
motivation: 在线宣传检测流程在数据收集和模型推理等阶段存在显著隐私风险，且现有方法往往缺乏对法律合规性的考量。
method: 提出PROMPT框架，通过效用函数量化风险缓解策略，并引入合规性评分标准及针对Transformer模型的扰动实验。
result: 评估显示多数现有流程不符合隐私法规，且实验证明隐私保护程度与模型F1分数之间存在明显的单调权衡关系。
conclusion: 本研究通过形式化风险防御映射和合规性审计指标，为开发兼顾隐私保护与检测效能的宣传检测系统奠定了理论与实验基础。
---
