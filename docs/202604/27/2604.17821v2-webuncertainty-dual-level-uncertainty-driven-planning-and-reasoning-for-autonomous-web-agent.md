...
tldr: 针对现有Web智能体在复杂动态任务中规划僵化和推理幻觉的问题，本文提出WebUncertainty框架。该框架引入双层不确定性驱动机制：任务层通过自适应规划选择模式应对未知环境，动作层利用蒙特卡洛树搜索（MCTS）结合置信度诱导的动作不确定性（ConActU）策略，量化偶然和认知不确定性以优化决策。实验证明其在WebArena和WebVoyager基准上优于现有SOTA模型。
selection_source: fresh_fetch
motivation: 现有Web智能体在处理长程复杂任务时，受限于僵化的规划策略和易产生幻觉的推理过程。
method: 提出双层不确定性驱动框架，包含任务级自适应规划机制和基于MCTS及ConActU策略的动作级推理机制。
result: 在WebArena和WebVoyager等主流基准测试中，WebUncertainty的性能显著超越了现有的最先进基准模型。
conclusion: 通过量化并利用规划与推理中的双层不确定性，可以显著提升自主Web智能体在真实网页环境中的决策稳健性和任务成功率。
---
