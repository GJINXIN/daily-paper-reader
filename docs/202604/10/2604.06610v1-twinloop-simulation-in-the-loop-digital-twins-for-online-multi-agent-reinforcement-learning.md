...
tldr: 针对多智能体系统在环境变化时在线学习收敛慢、试错成本高的问题，本文提出TwinLoop框架。该框架利用“仿真在环”数字孪生技术，在检测到环境偏移时，通过数字孪生重构系统状态并进行加速策略优化，随后将更新参数同步回物理系统。在车载边缘计算场景下的实验证明，该方法显著提升了系统的自适应效率，减少了对昂贵在线试错的依赖。
selection_source: fresh_fetch
motivation: 传统的去中心化在线学习在环境变化时需要大量的试错才能恢复性能，导致适应效率低下。
method: 提出TwinLoop框架，通过数字孪生进行状态重构和仿真“假设”分析，实现策略的离线加速改进与在线同步。
result: 在车载边缘计算任务卸载场景中，TwinLoop显著提高了环境偏移后的适应速度，并降低了在线学习成本。
conclusion: 数字孪生技术能有效辅助多智能体强化学习，通过仿真预演提升物理系统在动态环境下的鲁棒性与学习效率。
---

## 摘要
分布式在线学习使信息物理多智能体系统能够实现运行时自适应，但当运行条件发生变化时，已学习的策略通常需要大量的试错交互才能恢复性能。为了解决这一问题，我们提出了 TwinLoop，这是一个用于在线多智能体强化学习的仿真在环数字孪生框架。当发生上下文偏移时，数字孪生会被触发以重构当前系统状态，从最新的智能体策略进行初始化，并通过仿真假设分析（what-if analysis）执行加速策略改进，最后将更新后的参数同步回物理系统中的智能体。我们在工作负载和基础设施条件不断变化的车载边缘计算任务卸载场景中对 TwinLoop 进行了评估。结果表明，数字孪生可以提高偏移后的自适应效率，并减少对昂贵的在线试错的依赖。
