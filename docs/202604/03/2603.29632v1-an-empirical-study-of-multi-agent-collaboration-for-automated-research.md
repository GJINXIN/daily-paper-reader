...
tldr: 本研究针对自动化科研中的多智能体协作框架进行了系统性实证研究。通过在受控测试平台上对比单智能体与两种多智能体架构（子智能体并行模式与专家团队协作模式），发现在固定计算预算下，不同架构在操作稳定性和理论深度之间存在权衡。研究揭示了子智能体模式适合广度搜索，而团队模式适合深度重构，为未来自动化科研系统的动态架构设计提供了重要指导。
selection_source: fresh_fetch
motivation: 旨在探索在自动化科研任务中，何种多智能体协作框架能最有效地克服单模型认知瓶颈并提升优化效率。
method: 在受控实验环境下，对比了单智能体、子智能体并行探索架构以及专家团队分工协作架构在机器学习优化任务中的表现。
result: 子智能体模式在限时任务中表现出极高的运行韧性和搜索效率，而专家团队模式虽操作较脆弱，但在长时计算下能实现更深层的理论对齐。
conclusion: 未来自动化科研系统应采用动态路由架构，根据任务复杂度和实时需求灵活调整智能体间的协作结构。
---

## 摘要
随着人工智能智能体（AI agents）的演进，学术界正迅速从单一的大语言模型（LLMs）转向多智能体系统（MAS），以克服自动化研究中的认知瓶颈。然而，针对这些自主智能体的最优多智能体协调框架在很大程度上仍未得到探索。在本文中，我们提出了一项系统的实证研究，调查了不同多智能体结构在自动化机器学习优化中的比较效能。利用一个配备了 Git 工作树隔离和显式全局内存的、受严格控制的基于执行的测试平台，我们将单智能体基准与两种多智能体范式进行了对比：子智能体架构（具有事后整合的并行探索）和智能体团队架构（具有执行前交接的专家协作）。通过在严格固定的计算时间预算下评估这些系统，我们的研究结果揭示了运行稳定性与理论推敲之间的一种根本性权衡。子智能体模式表现为一个具有高韧性、高吞吐量的搜索引擎，最适合在严格的时间限制下进行广泛且浅层的优化。相反，智能体团队拓扑结构由于多作者代码生成而表现出更高的运行脆弱性，但在延长计算预算的情况下，能够实现复杂架构重构所需的深度理论对齐。这些实证见解为设计未来的自动化研究系统提供了可操作的指导方针，提倡采用能够根据实时任务复杂度调整其协作结构的动态路由架构。
