...
tldr: 针对工业网络中联邦学习面临的客户端行为不一致及恶意攻击等挑战，本文提出了一种轻量级的代理信任协调方法。该方法在服务端引入代理信任控制层，通过观察系统信号并进行自主推理，实现动态的信任调整。该框架在不增加通信开销或修改客户端训练的前提下，显著提升了联邦学习在复杂工业环境中的稳定性和鲁棒性。
selection_source: fresh_fetch
motivation: 传统的联邦学习信任机制多依赖固定参数或简单启发式规则，难以应对工业网络中动态变化的运行环境和潜在的恶意干扰。
method: 提出一种服务端代理信任控制层，通过解耦观察、推理与行动，利用自适应阈值和自主决策机制对客户端信任度进行动态干预。
result: 该方法能够在不改变客户端训练流程且不增加额外通信负担的情况下，有效识别并缓解不稳定因素对模型训练的影响。
