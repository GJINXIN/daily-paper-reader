...
tags: ["query:mas-ss"]
score: 6.0
evidence: 具身多智能体系统的语义通信框架
tldr: "针对具身智能体在带宽受限环境下的通信效率问题，本文提出了AgentComm语义通信框架。该框架利用大语言模型提取任务相关的语义内容以压缩消息，并结合重要性感知传输策略和任务知识库，在保证任务性能的同时显著降低物理层传输开销。实验表明，该方法在几乎不损失任务完成度的前提下，实现了约50%的带宽节省。"
selection_source: fresh_fetch
motivation: 现有的智能体通信方法缺乏在减少物理层传输开销的同时保留任务语义的有效机制。
method: 提出了一种结合LLM语义处理器、重要性感知传输策略和任务特定知识库的语义通信框架。
result: "实验结果显示，该框架在保持任务性能的同时，比传统传输方案减少了近50%的带宽消耗。"
conclusion: 语义通信是提升具身智能体在受限网络环境下协作效率的有效途径。
---

## 摘要
随着代理型人工智能（AI）系统的日益广泛部署，对高效智能体间通信的需求不断增加，特别是在带宽受限的无线链路环境下。在具身智能应用中，智能体必须在严格的延迟和可靠性约束下交换任务相关信息。现有的智能体通信方法主要侧重于连接性和协议效率，但缺乏在保留任务语义的同时减少物理层传输开销的有效机制。为应对这一挑战，本文提出了一种语义智能体通信框架，旨在降低通信开销的同时，维持任务性能和智能体间的共享理解。首先，引入了一种基于大语言模型（LLM）的语义处理器，通过提取任务相关的语义内容来重组和压缩智能体生成的消息。为了应对激进的消息缩减所带来的信息丢失，本文开发了一种重要性感知语义传输策略，根据任务重要性自适应地保护语义组件。此外，还引入了特定任务知识库作为长期语义记忆，以支持重复性任务，并在性能损失极小的情况下进一步降低带宽消耗。实验结果和消融实验表明，与传统传输方案相比，所提框架在任务完成性能损失微乎其微的前提下，实现了近 50% 的带宽缩减。
