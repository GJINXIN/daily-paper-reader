...
## 摘要
大语言模型（LLM）驱动的多智能体框架通过多角色协作来处理复杂的推理任务。然而，现有方法通常面临推理不稳定的问题，即单个智能体的错误会通过协作被放大，从而损害整体性能。目前的研究主要集中在增强高能力智能体或抑制不可靠输出来提高框架有效性，而对限制性能的智能体进行系统性识别和强化的关注较少。为了填补这一空白，我们基于短板原理（weak-link principle）提出了 WORC，这是一种用于多智能体推理与协作的短板优化框架。WORC 遵循两阶段工作流：在弱智能体定位阶段，通过构建任务特征，并利用在群体智能算法（SIA）识别的最优配置上训练的基于元学习的权重预测器，实现从任务特征到智能体性能权重的零样本映射，其中预测权重最低的智能体被识别为弱智能体；在短板优化阶段，一种不确定性驱动的分配策略为弱智能体分配额外的推理预算，较低的预测权重将获得更大的重复采样配额，以补偿其可靠性缺陷。实验结果表明，WORC 在推理基准测试中达到了 82.2% 的平均准确率，同时提升了框架稳定性和跨架构泛化能力。这表明，补偿短板而非仅仅强化长处，能够有效增强多智能体系统的鲁棒性。
