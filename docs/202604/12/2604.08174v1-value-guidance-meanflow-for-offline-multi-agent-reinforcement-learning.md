...
method: 提出VGM^2P框架，利用全局优势值引导条件行为克隆，并结合MeanFlow与无分类器引导技术实现高效的策略训练与推理。
result: 在离散和连续动作空间的任务实验中，VGM^2P仅通过条件行为克隆便实现了与当前最先进方法相当的性能。
conclusion: VGM^2P通过结合均值流与价值引导，为离线多智能体强化学习提供了一种高效、稳健且表达能力强的策略学习方案。
---

## 摘要
离线多智能体强化学习 (MARL) 旨在从预先收集的数据集中学习最优联合策略，这需要在最大化全局收益与缓解离线数据带来的分布偏移之间进行权衡。最近的研究使用扩散或流生成模型来捕捉智能体之间复杂的联合策略行为；然而，它们通常依赖于多步迭代采样，从而降低了训练和推理效率。尽管进一步的研究通过蒸馏等方法提高了采样效率，但其对行为正则化系数仍然敏感。为了解决上述问题，我们提出了价值引导多智能体 MeanFlow 策略 (VGM$^2$P)，这是一个简单且有效的基于流的策略学习框架，能够通过对系数不敏感的条件行为克隆实现高效的动作生成。具体而言，VGM$^2$P 使用全局优势值来引导智能体协作，将最优策略学习视为条件行为克隆。此外，为了提高多智能体场景下的策略表达能力和推理效率，它在策略训练和执行中均利用了无分类器引导的 MeanFlow。在离散和连续动作空间任务上的实验表明，即使仅通过条件行为克隆进行训练，VGM$^2$P 也能高效地达到与当前最先进方法相当的性能。
