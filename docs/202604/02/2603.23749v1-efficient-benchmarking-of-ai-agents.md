...
tldr: "针对AI智能体评估成本高昂的问题，本研究探讨了如何通过任务子集在降低成本的同时保持排名准确性。研究发现，尽管绝对分数预测受框架偏移影响较大，但排名预测相对稳定。提出了一种基于项目反应理论的优化协议，即仅在历史通过率为30-70%的中等难度任务上进行评估。该方法在减少44-70%评估任务量的同时，实现了比随机采样更可靠的排名，为高效基准测试提供了新思路。"
selection_source: fresh_fetch
motivation: AI智能体评估涉及多步推理和工具调用，全量基准测试成本极高，亟需高效的评估方案。
method: "提出一种无需优化的协议，仅选取历史通过率在30%至70%之间的中等难度任务进行评估。"
result: "该方法在减少44-70%任务量的基础上，保持了极高的排名保真度，且在框架和时间偏移下表现稳健。"
conclusion: 可靠的排行榜排名并不需要全量基准测试，通过筛选中等难度任务即可实现高效且准确的智能体评估。
---

## 摘要
在全面基准上评估 AI 智能体（AI agents）成本高昂，因为每次评估都需要涉及工具使用和多步推理的交互式展开（interactive rollouts）。我们研究了较小的任务子集是否能在大幅降低成本的情况下保持智能体排名。与静态语言模型基准不同，智能体评估受脚手架驱动的分布偏移（scaffold-driven distribution shift）影响，因为性能取决于封装底层模型的框架。通过对 8 个基准、33 个智能体脚手架和 70 多个模型配置的研究，我们发现绝对分数预测在这种偏移下会退化，而排名预测则保持稳定。利用这种不对称性，我们提出了一种简单的无优化协议：仅在具有中间历史通过率（30-70%）的任务上评估新智能体。这种受项目反应理论（Item Response Theory）启发的难度中等过滤器，在保持脚手架和时间偏移下的高排名保真度的同时，将评估任务数量减少了 44-70%。它提供了比随机采样更可靠的排名（随机采样在不同种子间表现出高方差），并且在分布偏移下优于贪婪任务选择。这些结果表明，可靠的排行榜排名并不需要全基准评估。
