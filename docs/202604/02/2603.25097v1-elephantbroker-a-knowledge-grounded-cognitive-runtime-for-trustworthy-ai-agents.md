...
tldr: ElephantBroker是一个开源认知运行时，旨在解决LLM智能体在复杂任务中缺乏知识溯源和信任机制的问题。它通过整合Neo4j知识图谱和Qdrant向量数据库，构建了包含存储、检索、评分、验证、保护和学习的完整认知闭环。该系统引入了多维评分引擎、AI防火墙和分层访问控制，确保了智能体记忆的持久性、可验证性和安全性，适用于从轻量级到企业级的多种应用场景。
selection_source: fresh_fetch
motivation: 现有的LLM智能体记忆系统通常依赖简单的向量检索，缺乏对存储知识来源和可信度的追踪机制，难以满足高风险场景的需求。
method: 提出一种结合知识图谱与向量存储的架构，通过混合检索、多维评分、证据验证及AI防火墙等模块实现可信的认知循环。
result: 通过超过2200项涵盖单元、集成和端到端的测试验证了系统的正确性，并支持多层级部署和人类监督。
conclusion: ElephantBroker为构建具备企业级安全性和可审计性的可信AI智能体提供了一个模块化、可扩展的认知运行时框架。
---
