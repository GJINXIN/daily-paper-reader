...
## 摘要
现实场景日益要求多个具身智能体在具身约束下的动态环境中进行协作，因为许多任务超出了单个智能体的能力范围。大语言模型（LLM）的最新进展通过推理、规划和自然语言通信实现了高层认知协调。然而，现有基准难以对具身多智能体系统中此类协作如何出现、展开以及如何促进任务成功进行细粒度分析。在本文中，我们介绍了 EmCoop，这是一个用于研究基于 LLM 的具身多智能体系统协作的基准框架。我们的框架将高层认知层与底层具身交互层分离，使我们能够通过随时间变化的交织动态来表征智能体间的协作。针对受协作约束的具身任务，我们提出了可推广的过程级指标，用于诊断协作质量和失败模式，而不仅仅局限于最终的任务成功率。我们在两个具身环境中实例化了我们的框架，这些环境可扩展到任意数量的智能体并支持多种通信拓扑，并利用这些实例展示了 EmCoop 如何实现对不同团队规模和任务设置下协作动态的系统分析。项目网页见：https://happyeureka.github.io/emcoop。
