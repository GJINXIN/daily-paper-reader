...
tldr: 针对大语言模型在复杂多步推理中的局限性，受人类认知“全局工作空间理论”启发，本文提出BIGMAS框架。该框架将专用LLM智能体组织为动态有向图，通过中心化共享空间进行协作。通过GraphDesigner构建任务拓扑和Orchestrator进行全局调度，BIGMAS在多个推理任务中显著提升了标准LLM和推理模型（LRM）的性能，证明了多智能体架构与模型推理增强的互补性。
selection_source: fresh_fetch
motivation: 现有的LLM和推理模型在处理极复杂任务时仍面临准确率崩溃，单纯扩展模型规模不足以解决复杂推理问题。
method: 提出BIGMAS框架，利用动态图结构组织智能体，并通过中心化共享工作空间和全局编排器实现高效协作。
result: 在Game24、Six Fives等任务上的实验表明，BIGMAS优于ReAct和思维树等基准，且能进一步提升前沿LLM的推理能力。
