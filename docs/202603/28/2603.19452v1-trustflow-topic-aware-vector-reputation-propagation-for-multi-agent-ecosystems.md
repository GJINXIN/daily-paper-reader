...
tldr: TrustFlow 是一种针对多智能体生态系统的主题感知向量声誉传播算法。它将声誉表示为多维向量而非传统标量，通过主题门控传输算子在交互图中传播，并利用收缩映射定理保证收敛。该方法能有效抵御女巫攻击和声誉洗钱，支持通过点积直接查询，在多标签分类任务中表现优异，显著提升了复杂网络中声誉评估的准确性与鲁棒性。
selection_source: fresh_fetch
motivation: 传统的标量声誉系统难以处理多维度的专业领域评价，且容易受到女巫攻击和声誉操纵的影响。
method: 提出 TrustFlow 算法，利用 Lipschitz-1 传输算子和信息论门控机制，在交互图中传播与内容嵌入相关联的多维声誉向量。
result: "在密集图中实现了 98% 的 Precision@5，且在面对女巫攻击和投票环时，精度下降仅在 4 个百分点以内。"
conclusion: TrustFlow 通过向量化声誉和主题感知机制，为多智能体系统提供了一种高精度、抗攻击且可直接查询的信任评估框架。
---

## 摘要
我们引入了 TrustFlow，这是一种信誉传播算法，它为每个软件智能体分配一个多维信誉向量，而非标量分数。信誉通过交互图进行传播，利用主题门控转移算子根据内容嵌入对每条边进行调制，并通过收缩映射定理保证收敛至唯一的固定点。我们开发了一系列 Lipschitz-1 转移算子和可组合的信息论门控，在稠密图上实现了高达 98% 的多标签 Precision@5，在稀疏图上实现了 78%。在涵盖 8 个领域的 50 个智能体的基准测试中，TrustFlow 能够抵御女巫攻击、信誉洗钱和投票环，其精度影响最高仅为 4 个百分点。与 PageRank 和主题敏感 PageRank 不同，TrustFlow 生成的向量信誉可以直接通过与用户查询处于同一嵌入空间中的点积进行查询。
