...
## 摘要
图滤波器利用拓扑信息处理网络化数据，而现有方法主要研究固定图，忽略了图通常会随着节点以未知模式不断加入而扩张。后者需要开发基于滤波器的决策范式，以充分考虑演化和不确定性。现有方法依赖于预先设计的滤波器或在线学习，局限于仅考虑过去或当前信息的短视视角。为了考虑未来的影响，我们提出了一种用于过滤网络化数据的随机序列决策框架，该框架采用一种使滤波适应扩张网络的策略。通过将滤波器移位表示为智能体，我们将滤波器建模为多智能体系统，并按照多智能体强化学习训练策略。这考虑了长期奖励，并通过序列决策捕捉扩张动态。此外，我们开发了一种上下文感知图神经网络来参数化该策略，该网络根据图和智能体的信息来调整滤波器参数。在从冷启动推荐到 COVID 预测的合成和真实数据集上的实验表明，与批量和在线滤波替代方案相比，使用序列决策视角具有显著优势。
