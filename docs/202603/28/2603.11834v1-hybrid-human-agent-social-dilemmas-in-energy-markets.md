...
motivation: 旨在解决能源市场中由于需求定价机制导致的消费者用电拥堵和社会困境，探索如何通过自主智能体促进群体协调。
method: 引入能够识别全局信号的智能体，并结合演化动力学与强化学习实验，模拟分析混合群体在不同采用率下的决策行为。
result: 实验证明智能体能显著提升协调成功率，且早期采用者在改善整体结果的同时不会面临结构性损失。
conclusion: AI代理能有效缓解能源市场的社会困境，但部署时需考虑采用者与非采用者之间收益分配的不对称性。
---
