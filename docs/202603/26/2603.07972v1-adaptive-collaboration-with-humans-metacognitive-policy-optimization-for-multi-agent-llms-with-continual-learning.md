...
tldr: 针对多智能体系统在处理超出预训练知识范围的任务时表现脆弱的问题，本文提出了HILA框架。该框架引入元认知策略，使智能体能自主判断何时寻求人类专家帮助。通过双环策略优化，内环利用GRPO优化求助决策，外环通过持续学习将专家反馈转化为推理能力的提升。实验证明，HILA在数学和复杂问题解决基准测试中显著优于现有系统，为构建可协作且持续进化的智能体奠定了基础。
selection_source: carryover_cache
motivation: 现有的多智能体系统受限于预训练知识的静态边界，在面对新挑战或未知领域时容易出现集体失效。
method: 提出HILA框架，采用双环策略优化：内环优化何时求助人类的决策，外环通过持续学习将人类反馈转化为模型能力的增长。
result: 在具有挑战性的数学和问题解决基准测试中，HILA的表现一致优于现有的先进多智能体系统。
conclusion: HILA为人类与智能体的高效协作及智能体的持续自我进化提供了一种原则性的范式。
---

## 摘要
虽然扩展单个大语言模型（LLM）已取得显著进展，但下一个前沿领域在于通过多智能体系统（MAS）扩展协作。然而，纯自主的多智能体系统仍然是“闭世界”系统，受限于预训练模型静态的知识视野。这种局限性使得它们在处理需要超出训练数据知识的任务时表现脆弱，在面对新挑战时往往会导致集体失败。为了解决这一问题，我们提出了人机回环多智能体协作（HILA）框架，这是一种人机协作的规范范式。HILA 训练智能体学习一种元认知策略，用于管理何时自主解决问题以及何时求助于人类专家。为了实施这一策略，我们引入了双环策略优化（Dual-Loop Policy Optimization），将即时决策与长期能力增长解耦。内环应用带有成本感知奖励的群体相对策略优化（GRPO）来优化求助决策，而外环则实施持续学习，将专家反馈转化为高质量的监督信号，以增强智能体的推理能力。在具有挑战性的数学和问题解决基准测试上的实验表明，配备双环策略优化的 HILA 始终优于先进的多智能体系统，为协作式且持续改进的智能体系统奠定了规范基础。
