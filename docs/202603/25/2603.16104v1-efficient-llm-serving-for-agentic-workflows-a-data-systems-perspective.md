...
## 摘要
智能体工作流由一系列相互依赖的大语言模型（LLM）调用组成，已成为现代 AI 系统中的主流工作负载。由于推测性执行和并行探索，这些工作流在重叠的提示词（prompts）和中间结果中表现出大量的冗余。现有的 LLM 推理服务系统（如 vLLM）专注于优化单个推理调用，而忽略了跨调用之间的依赖关系，导致了显著的效率低下。本文从数据系统的视角重新审视了 LLM 和智能体推理服务，并引入了 Helium。Helium 是一个感知工作流的推理服务框架，它将智能体工作负载建模为查询计划（query plans），并将 LLM 调用视为一等算子（first-class operators）。Helium 集成了主动缓存和感知缓存的调度策略，以最大限度地实现提示词、KV 状态和工作流之间的复用。通过这些技术，Helium 将经典的查询优化原则与 LLM 推理服务相结合，在各种工作负载上相比最先进的智能体推理服务系统实现了高达 1.56 倍的加速。我们的结果表明，跨工作流的端到端优化对于构建可扩展且高效的基于 LLM 的智能体至关重要。
