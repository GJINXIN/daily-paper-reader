...
## 摘要
领域专家拥有难以通过显式规范轻松表达的隐性知识。当专家通过纠正术语、重构论点和调整重点来修改 AI 生成的产物时，这些编辑揭示了在传统的基于提示词的交互中处于潜伏状态的领域理解。当前系统将此类修改视为终点修正，而非能够重塑后续推理的隐性规范。我们提出了上下文介导的领域自适应，这是一种将用户对系统生成产物的修改作为隐性领域规范，从而重塑大语言模型（LLM）驱动的多智能体推理行为的范式。通过我们的系统 Seedentia（一个基于 Web 的多智能体意义构建框架），我们展示了生成的产物与系统推理之间的双向语义链接。我们的方法实现了规范引导，使模糊的初始提示词通过迭代的人机协作演变为精确的领域规范；通过逆向工程用户编辑实现隐性知识转移；以及基于观察到的修正模式使智能体行为进行自适应的上下文学习。我们展示了对领域专家进行评估的结果，这些专家生成并修改了学术论文中的研究问题。我们的系统从用户修改中提取了 46 条领域知识条目，证明了通过编辑模式捕捉隐性专业知识的可行性，尽管有限的样本量限制了关于系统性质量提升的结论。
