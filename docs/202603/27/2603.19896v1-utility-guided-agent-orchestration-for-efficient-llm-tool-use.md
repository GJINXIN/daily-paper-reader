...
tldr: 本研究针对LLM智能体在工具使用中面临的质量与成本权衡问题，提出了一种效用引导的编排策略。传统方法如ReAct虽灵活但成本高昂，而固定工作流则缺乏灵活性。该方法通过综合评估预估收益、执行成本、不确定性和冗余度，动态选择响应、检索或工具调用等动作。实验证明，该框架能有效控制智能体行为，在保证任务质量的同时显著优化资源消耗，为智能体的高效编排提供了可控且可分析的方案。
selection_source: fresh_fetch
motivation: 旨在解决LLM智能体在工具调用过程中，答案质量与执行成本（如Token消耗、延迟）之间难以平衡的矛盾。
method: 提出一种效用引导的编排策略，通过量化评估收益、成本、不确定性和冗余来动态决策智能体的下一步动作。
result: 实验显示，显式的编排信号能显著优化智能体行为，在多种基准测试中实现了比ReAct和固定工作流更优的质量-成本权衡。
conclusion: 轻量级的效用设计为工具使用型智能体提供了一种防御性强且实用的控制机制，增强了编排过程的可分析性。
---
