...
## 摘要
设计高性能系统启发式算法是一个创造性的迭代过程，需要专家提出假设并执行多步概念转变。虽然大语言模型（LLMs）在自动化这一循环方面展现出潜力，但由于两种关键的失败模式，它们在处理复杂系统问题时仍面临困难：进化邻域偏差（evolutionary neighborhood bias）和连贯性天花板（coherence ceiling）。进化方法通常依赖标量基准分数，从而陷入局部最优，在需要协调的多步变更时往往会失败。相反，现有的代理（agentic）框架在长周期内会遭受上下文退化，或者无法在独立的运行之间积累知识。我们提出了 Engram，这是一种代理研究员架构，通过将长周期探索与单一上下文窗口的限制解耦，解决了上述局限性。Engram 将探索组织为一系列代理，这些代理迭代地设计、测试并分析机制。在每次运行结束时，代理会将代码快照、日志和结果存储在持久化存档（Archive）中，并将高层建模见解提炼为简洁、持久的研究摘要（Research Digest）。随后的代理将从全新的上下文窗口开始，通过阅读研究摘要来基于先前的发现进行构建。我们发现 Engram 在多个领域表现出优异的性能，包括多云组播（multi-cloud multicast）、LLM 推理请求路由，以及自然语言查询数据库中的 KV 缓存重用优化。
