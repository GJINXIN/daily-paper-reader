...
tldr: CoMAI是一个针对AI面试评估的协作式多智能体框架。针对单体LLM系统在鲁棒性和公平性上的不足，该框架采用任务分解架构，通过有限状态机协调问题生成、安全、评分和总结四个专业智能体。它能有效防御提示注入，支持自适应难度调整和结构化评分，显著提升了面试评估的准确性、公平性和用户满意度。
selection_source: carryover_cache
motivation: 旨在解决现有单体LLM面试系统在评估鲁棒性、安全性和公平性方面存在的局限。
method: 提出一种基于有限状态机协调的模块化多智能体架构，包含专门负责生成、安全、评分和总结的四个智能体。
result: "实验显示该框架达到了90.47%的准确率和84.41%的候选人满意度，有效降低了主观偏见并增强了安全性。"
conclusion: CoMAI为AI驱动的面试评估提供了一个鲁棒、公平且可解释的新范式，具有广泛的应用前景。
---
